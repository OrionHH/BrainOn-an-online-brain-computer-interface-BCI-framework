...
def LDA_kernel(X1: ndarray, X2: ndarray):
    ...
    avg_feats1, avg_feats2 = X1.mean(axis=0, keepdims=True), X2.mean(axis=0, keepdims=True)
...
class STDA(BaseEstimator, TransformerMixin, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.L > n_chs or self.L > n_features:
            raise ValueError('The parameter L must be less than n_sample or n_features.')
        ...
        while 1:
            ...
            for k in range(1, n_classes+1):
                if k == 1:
                    ...
                    Y_mat_c1, Y_mat_c2 = np.matmul(W1[-1].T, X1).transpose((0, 2, 1)), np.matmul(W1[-1].T, X2).transpose((0, 2, 1))

                Y_bar_c1, Y_bar_c2 = Y_mat_c1.mean(axis=0), Y_mat_c2.mean(axis=0)
                Y_bar_all = ((Y_bar_c1 * n_samples_c1) + (Y_bar_c2 * n_samples_c2)) / (n_samples_c1 + n_samples_c2)

                # construct Sb
                y1_tmp, y2_tmp = Y_bar_c1 - Y_bar_all, Y_bar_c2 - Y_bar_all
                Sb = n_samples_c1 * (y1_tmp @ y1_tmp.T) + n_samples_c2 * (y2_tmp @ y2_tmp.T)
                ...
                if k == 1:
                    W1.append(eig_vecs[:, loc_idx][:, :self.L])  # return indices in ascending order and reverse
                else:
